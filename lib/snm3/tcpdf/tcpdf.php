...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		if ($ln AND (!($cell AND ($dom[$key-1]['value'] == 'table')))) {
			...
			if ($this->y < $maxbottomliney) {
				...
				//AUIT 
				//Mage::log("tcpdf maxbottomliney : $maxbottomliney ");
				//AUIT				$this->y = $maxbottomliney;				
			}
		}
		unset($dom);
	}
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		if ($tag['block']) {
			//AUIT START
			$prev = $dom[($dom[$key]['elkey'])];
			if ( isset($prev['value']) && $prev['value'] && !$prev['tag'])
			{
				if ( $tag['value'] != 'br')
				{
					$this->Ln($this->lasth,$cell);
					$this->htmlvspace = 0;
				}
			}
			if ( in_array($tag['value'], array('ul', 'ol', 'li')) ) {
				$firsttag=true;
				if ( $tag['value'] == 'li' and $prev['value'] != 'ul')
				{
					$firsttag=false;
				}				
			}
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		...
		if ($tag['block']) {
			...
			//AUIT START
			$prev = $dom[($dom[$key]['elkey'])];
			if ( $parent['tag'] && $parent['block'] ) //&& $prev['block'])
			{
				if ( $tag['value'] == 'ul' || $tag['value'] == 'ol')
					$hbz = 0;
				else if ( $prev['block'] )
				{
					$hbz = 0;
	...
	*/
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false,$mainscale=-1) {
		...
		if ( $mainscale > 0 )
		{
			$svgscale_x = $mainscale;
			$svgscale_y = $mainscale;
		}
